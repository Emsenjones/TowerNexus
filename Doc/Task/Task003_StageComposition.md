...
Implement StageDefinition-driven battle composition so one selected Stage supplies its Player maximum health, Map template, MonsterWaveConfig, Tower Draft pool, and Tower Upgrade Draft pool before battle runtime begins.
...
- One positive Player Max Health value
...
Receive Selected StageDefinition
    ...
    -> Supply Player Max Health
    -> Supply Tower And Upgrade Draft Pools
    -> Initialize Fresh Player Battle State At Full Health
...
| PlayerSystem | Selected positive Player Max Health |
...
- TowerDefinitionDatabase and TowerUpgradeDatabase are removed after DraftSystem and serialized prefab references migrate to Stage pools.
...
- Recomposition must not retain prior Player maximum health, current health, progress, Draft pools, Wave state, occupied nodes, or Active Map references.
...
- Assign its positive Player Max Health.
...
- Confirm the GameManager prefab contains no old fixed Map, Wave, Database component, or missing-script reference.
...
- The selected Stage's positive Player Max Health becomes both the Player runtime maximum and fresh current health.
...
Implementation static validation completed on 2026-07-24:

- `StageDefinition.cs` and `StageCompositionController.cs` were both explicitly included in the main Unity assembly compilation.
- Main Unity assembly compilation passed with zero warnings and zero errors.
- Stage composition owns Stage validation and Active Map lifecycle, while BattleRuntimeCoordinator encapsulates consumer preparation, battle commit, defeat stop, and prepared-runtime release.
- Runtime scripts no longer read `TowerDefinitionDatabase`, `TowerUpgradeDatabase`, fixed serialized MonsterWaveConfig references, serialized Map fallbacks, scene-wide Map discovery, or scene-wide Tower discovery.
- `TowerDefinitionDatabase` and `TowerUpgradeDatabase` scripts, metadata, and GameManager prefab components were removed after the Lv1 Stage pools were serialized.
- Player maximum health is now authored by StageDefinition, validated as positive, and transactionally applied before battle commit.
- Both new Stage scripts are permanent generated `Assembly-CSharp.csproj` entries.
- The current GameManager prefab retains `TowerUpgradeSystem` while removing only its obsolete co-located Database component.

Remaining Unity handoff:

- Open Unity and confirm both new Stage scripts import without serialization or Inspector errors.
- Confirm the removed Database scripts do not produce missing-script components after Unity imports the edited GameManager prefab.
- Search for remaining runtime reads of fixed MonsterWaveConfig, scene-wide Map discovery, and stale Active Map references.
- Compose two controlled StageDefinitions sequentially with different Player Max Health values and confirm Map, Wave, Draft pool, Player state, and cleanup isolation.
- Validate missing Map, invalid Map, missing Wave, duplicate pool entries, invalid definition, and family-coherence failures.
- Validate that non-positive Player Max Health prevents battle preparation and leaves gameplay stopped.
