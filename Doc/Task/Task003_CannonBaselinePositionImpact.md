...
- Use `ProjectileConfig.hitDistanceThreshold` as Arc arrival tolerance and local Monster-query radius.
...
- Update the Cannon ProjectileConfig so `hitDistanceThreshold` is positive and appropriate for both arrival tolerance and local direct-target search.
